        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <input 
                className="w-full bg-black border border-zinc-800 p-4 rounded-xl outline-none focus:border-red-500"
                placeholder="E-posta" value={formData.email}
                onChange={(e) => setFormData({...formData, email: e.target.value})}
              />
              <button onClick={handleStep1} className="w-full bg-red-500 p-4 text-white font-black uppercase italic rounded-xl">
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <input 
                className="w-full bg-black border border-zinc-800 p-4 rounded-xl text-center text-2xl font-mono tracking-widest outline-none focus:border-red-500"
                placeholder="000000" maxLength={6}
                onChange={(e) => setEnteredCode(e.target.value)}
              />
              <button onClick={handleStep2} className="w-full bg-red-500 p-4 text-white font-black uppercase italic rounded-xl">
          ...
          {step === 3 && (
            <motion.div key="s3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <input placeholder="Ad" className="bg-black p-4 rounded-xl border border-zinc-800 focus:border-red-500 outline-none" onChange={(e) => setFormData({...formData, firstName: e.target.value})} />
                <input placeholder="Soyad" className="bg-black p-4 rounded-xl border border-zinc-800 focus:border-red-500 outline-none" onChange={(e) => setFormData({...formData, lastName: e.target.value})} />
                <input placeholder="T.C. / Pasaport No" className="col-span-2 bg-black p-4 rounded-xl border border-zinc-800 focus:border-red-500 outline-none" onChange={(e) => setFormData({...formData, idNumber: e.target.value})} />
                <input type="date" className="bg-black p-4 rounded-xl border border-zinc-800 text-zinc-500 focus:border-red-500 outline-none" onChange={(e) => setFormData({...formData, birthDate: e.target.value})} />
                <input placeholder="Telefon" className="bg-black p-4 rounded-xl border border-zinc-800 focus:border-red-500 outline-none" onChange={(e) => setFormData({...formData, phone: e.target.value})} />
              </div>
            ...
            <motion.div key="s4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="flex items-center justify-between p-4 bg-black rounded-xl border border-zinc-800">
                  ...
                  <input type="checkbox" checked={formData.needsTransfer} onChange={(e) => setFormData({...formData, needsTransfer: e.target.checked})} className="accent-red-500" />
                </label>
                <label className="flex items-center justify-between p-4 bg-black rounded-xl border border-zinc-800">
                  <span>Motosiklet Kiralama (€250)</span>
                  <input type="checkbox" checked={formData.needsBikeRental} onChange={(e) => setFormData({...formData, needsBikeRental: e.target.checked})} className="accent-red-500" />
                </label>
              </div>
              ...
              <button onClick={handleStep3And4} className="w-full bg-red-500 p-4 text-white font-black rounded-xl uppercase italic">ÖDEMEYE GEÇ</button>
            </motion.div>
          )}

          {step === 5 && (
            <motion.div key="s5" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-6">
              <div className="w-20 h-20 bg-red-500 rounded-full flex items-center justify-center mx-auto text-white font-black">✓</div>
              ...
              <button onClick={() => window.location.href="/"} className="text-red-500 font-bold uppercase underline">Anasayfaya Dön</button>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
